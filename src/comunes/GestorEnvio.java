...
public class GestorEnvio {
	...
	public static Timer FlushTimer()
	{
	    ActionListener action = new ActionListener ()
	      {
	        ...
	        {
	        	for(Entry<Integer, Map<Long, Map<PrintWriter, String>>> data : GestorEnvio.getPacketBuffer().entrySet())
	        	{
	        		if(GestorEnvio.getPacketBuffer().get(data.getKey()).isEmpty()) continue;
	        		StringBuilder Totaldata = new StringBuilder();
	        		PrintWriter pw = null;
	        		for(Entry<Long, Map<PrintWriter, String>> s : GestorEnvio.getPacketBuffer().get(data.getKey()).entrySet())
	        		{
	        			for(Entry<PrintWriter, String> s2 : GestorEnvio.getPacketBuffer().get(data.getKey()).get(s.getKey()).entrySet())
	        			{
	        				Totaldata.append((s2.getValue())).append((char)0x00);
	        				if(pw != null && (pw.hashCode() == s2.getKey().hashCode())) continue;
	        				pw = s2.getKey();
	        			}
	        			GestorEnvio.set_BufferRemove(s.getKey()+",");
	        		}
	        		if(Totaldata.toString().isEmpty()) continue;
	        		for(String id : GestorEnvio.get_BufferRemove().split(","))
	        		{
	        			data.getValue().remove(Long.parseLong(id));
	        		}
	        		GestorEnvio.del_BufferRemove();
	    ...
	    return new Timer (MainServidor.CONFIG_SOCKET_TIME_COMPACT_DATA, action);
	}
