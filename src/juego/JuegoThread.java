...
public class JuegoThread implements Runnable
{
	...
	private void parse_enemyPacket(String packet)
	{
		...
		break;
		case 'L'://Liste
			GestorSalida.GAME_SEND_ENEMY_LIST(_perso);
		break;
		}
	}
	...
	private void Enemy_add(String packet)
	{
		...
		switch(packet.charAt(2))
		{
			case '%'://Nom de perso
				...
				Jugador P = Mundo.getPersoByName(packet);
				if(P == null)
				{
	...
	private void Zaapi_use(String packet)
	{
		...
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "183");
			return;
		}
	...
	private void guild_perco_join_fight(String packet) 
	{
		switch(packet.charAt(0))
		{
			case 'J'://Rejoindre
				...
				if(MainServidor.CONFIG_DEBUG) JuegoServidor.addToLog("[DEBUG] Percepteur INFORMATIONS : TiD:"+TiD+", FightID:"+FightID+", MapID:"+MapID+", CellID"+CellID);
	...
	private void guild_remove_perco(String packet) 
	{
		...
		GestorSalida.GAME_SEND_ERASE_ON_MAP_TO_MAP(_perso.get_curCarte(), IDPerco);
		GestorSQL.eliminar_recaudador(perco.getGuid());
		perco.DelPerco(perco.getGuid());
		for(Jugador z : _perso.get_guild().getMembers())
		{
			if(z.isOnline())
			{
				...
				str += perco.get_mapID()+"|";
				str += Mundo.getCarte((short)perco.get_mapID()).getX()+"|"+ Mundo.getCarte((short)perco.get_mapID()).getY()+"|"+_perso.get_name();
	...
	private void guild_add_perco() 
	{
		...
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "182");
			return;
		}
		if(Recaudador.GetPercoGuildID(_perso.get_curCarte().get_id()) > 0)//La carte possï¿½de un perco
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "1168;1");
			return;
		}
		if(_perso.get_curCarte().get_placesStr().length() < 5)//La map ne possï¿½de pas de "places"
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "113");
			return;
		}
		...
		Recaudador perco = new Recaudador(id, _perso.get_curCarte().get_id(), _perso.get_curCell().getID(), (byte)3, _perso.get_guild().get_id(), random1, random2, "", 0, 0);
		Mundo.addPerco(perco);
		GestorSalida.GAME_SEND_ADD_PERCO_TO_MAP(_perso.get_curCarte());
		GestorSQL.agregar_recaudador_en_mapa(id, _perso.get_curCarte().get_id(), _perso.get_guild().get_id(), _perso.get_curCell().getID(), 3, random1, random2);
		for(Jugador z : _perso.get_guild().getMembers())
		{
	...
	private void guild_enclo(String packet)
	{
		...
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "1135");
			return;
		}
		...
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "1159");
			return;
		}
	}
	...
	private void guild_house(String packet)
	{
		...
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "1136");
			return;
		}
		...
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "1137");
			return;
		}
	}
	...
	private void guild_promote(String packet)
	{
		...
		Jugador p = Mundo.getPersonnage(guid);	//Cherche le personnage a qui l'on change les droits dans la mï¿½moire
		...
		{
			int guildId = GestorSQL.isPersoInGuild(guid);	//Rï¿½cupï¿½re l'id de la guilde du personnage qui n'est pas dans la mï¿½moire
	...
	{
		if(_perso.get_guild() == null)return;
		Jugador P = Mundo.getPersoByName(name);
	...
	private void guild_join(String packet)
	{
		...
		{
		case 'R'://Nom perso
			Jugador P = Mundo.getPersoByName(packet.substring(1));
			...
			{
				GestorSalida.GAME_SEND_Im_PACKET(_perso, "155;"+(40+_perso.get_guild().get_lvl()));
				return;
			}
		...
		case 'E'://ou Refus
			...
			{
				Jugador p = Mundo.getPersonnage(_perso.getInvitation());
	...
	private void guild_infos(char c)
	{
		...
		break;
		case 'F'://Enclos
			GestorSalida.GAME_SEND_gIF_PACKET(_perso, Mundo.parseMPtoGuild(_perso.get_guild().get_id()));
		break;
		case 'G'://General
			GestorSalida.GAME_SEND_gIG_PACKET(_perso, _perso.get_guild());
		break;
		case 'H'://House
			GestorSalida.GAME_SEND_gIH_PACKET(_perso, House.parseHouseToGuild(_perso));
		break;
		case 'M'://Members
			GestorSalida.GAME_SEND_gIM_PACKET(_perso, _perso.get_guild(),'+');
		break;
		case 'T'://Perco
	...
	private void guild_create(String packet)
	{
		...
		try
		{
			...
			if(_perso.get_curCarte().get_id() == 2196)//Temple de crï¿½ation de guilde
			{
				...
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "14");
					return;
				}
	...
	private void parseMountPacket(String packet)
	{
		switch(packet.charAt(1))
		{
			case 'b'://Achat d'un enclos
				...
				Jugador Seller = Mundo.getPersonnage(MP.get_owner());
				if(MP.get_owner() == -1)
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "196");
					return;
				}
				if(MP.get_price() == 0)
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "197");
					return;
				}
				if(_perso.get_guild() == null)
				{
				...
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "198");
					return;
				}
				...
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "1103");
					return;
				}
				...
				for(Jugador z:_perso.get_curCarte().getPersos())
				{
			...
			case 's'://Vendre l'enclo
				...
				if(MP1.get_owner() == -1)
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "194");
					return;
				}
				if(MP1.get_owner() != _perso.get_GUID())
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "195");
					return;
				}
				MP1.set_price(price);
	...
	{
		Jugador Wife = Mundo.getPersonnage(_perso.getWife());
		if(Wife == null) return;
		if(!Wife.isOnline())
		{
			if(Wife.get_sexe() == 0) GestorSalida.GAME_SEND_Im_PACKET(_perso, "140");
			else GestorSalida.GAME_SEND_Im_PACKET(_perso, "139");
	...
	private void parseGroupPacket(String packet)
	{
		switch(packet.charAt(1))
		{
			...
			case 'F'://Suivre membre du groupe PF+GUID
				...
				Jugador P = Mundo.getPersonnage(pGuid);
			...
			case 'G'://Suivez le tous PG+GUID
				...
				Jugador P2 = Mundo.getPersonnage(pGuid2);
				
				if(P2 == null || !P2.isOnline()) return;
				
				if(packet.charAt(2) == '+')//Suivre
				{
					for(Jugador T : g2.getPersos())
					{
	...
	private void group_locate()
	{
		...
		for(Jugador GroupP : _perso.getGroup().getPersos())
		{
			if(!isFirst) str += "|";
	...
	private void group_quit(String packet)
	{
		...
		}else if(g.isChief(_perso.get_GUID()))//Sinon, c'est qu'il kick un joueur du groupe
		{
			...
			if(guid == -1)return;
			Jugador t = Mundo.getPersonnage(guid);
			g.leave(t);
	...
	private void group_invite(String packet)
	{
		...
		Jugador target = Mundo.getPersoByName(name);
	...
	private void group_refuse()
	{
		...
		Jugador t = Mundo.getPersonnage(_perso.getInvitation());
		if(t == null) return;
		t.setInvitation(0);
	...
	private void Object_drop(String packet)
	{
		...
		if(cellPosition < 0)
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "1145");
			return;
		}
		if(obj.getPosition() != Constantes.ITEM_POS_NO_EQUIPED)
		{
			...
			if(obj.getPosition() == Constantes.ITEM_POS_ARME 		||
				...
				obj.getPosition() == Constantes.ITEM_POS_NO_EQUIPED)
					GestorSalida.GAME_SEND_ON_EQUIP_ITEM(_perso.get_curCarte(), _perso);
		}
		...
		GestorSalida.GAME_SEND_GDO_PACKET_TO_MAP(_perso.get_curCarte(),'+',_perso.get_curCarte().getCase(_perso.get_curCell().getID()+cellPosition).getID(),obj.getTemplate().getID(),0);
	...
	private void Object_use(String packet)
	{
		...
		if(!obj.getTemplate().getConditions().equalsIgnoreCase("") && !Condiciones.validConditions(_perso,obj.getTemplate().getConditions()))
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "119|43");
			return;
		}
	...
	private synchronized void Object_move(String packet)
	{
		...
		try
		{
			...
			if(!Constantes.isValidPlaceForItem(obj.getTemplate(),pos) && pos != Constantes.ITEM_POS_NO_EQUIPED) // si mauvaise place
				return;
			if(!obj.getTemplate().getConditions().equalsIgnoreCase("") && !Condiciones.validConditions(_perso,obj.getTemplate().getConditions())) {
				GestorSalida.GAME_SEND_Im_PACKET(_perso, "119|43"); // si le perso ne vï¿½rifie pas les conditions diverses
				return;
			}
			...
			if ((objGUID == 9234) || (objGUID == 9233) || (objGUID == 9255) || (objGUID == 9256))
			{
				...
				if ((objGUID == 9233) || (objGUID == 9234)) 
					GestorSalida.GAME_SEND_ON_EQUIP_ITEM(_perso.get_curCarte(), _perso); // Si l'obvi ï¿½tait cape ou coiffe : packet au client
			...
			{
				GestorSalida.GAME_SEND_PM_MOD_PACKET_TO_GROUP(_perso.getGroup(),_perso);
			}
			GestorSalida.GAME_SEND_STATS_PACKET(_perso);
	...
	{	
		switch(packet.charAt(1))
		{
			...
			case 'q'://Mode marchand
		        ...
		        {
		        	GestorSalida.GAME_SEND_Im_PACKET(_perso, "125;5");
		        	return;
		        }
		        if (_perso.parseStoreItemsList().isEmpty())
		        {
		        	GestorSalida.GAME_SEND_Im_PACKET(_perso, "123");
		        	return;
		        }
		        ...
		        {
		        	if(z != null && z.isOnline())
		        		GestorSalida.GAME_SEND_MERCHANT_LIST(z, z.get_curCarte().get_id());
		        }
			break;
		...
		switch(packet.charAt(2))
		{
			case 'B': //Confirmation d'achat
				...
				if(curHdv.buyItem(ligneID,amount,Integer.parseInt(info[2]),_perso))
				{
					...
					_perso.refreshStats();
					GestorSalida.GAME_SEND_Ow_PACKET(_perso);
					GestorSalida.GAME_SEND_Im_PACKET(_perso,"068");//Envoie le message "Lot achetï¿½"
				}
				else
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso,"172");//Envoie un message d'erreur d'achat
				}
			break;
			...
			switch(c)
			{
				...
				case 'c'://Etable => Parcho(Echanger)
							...
							!_perso.getGuildMember().canDo(Constantes.G_OTHDINDE))
					{
						...
						GestorSalida.GAME_SEND_Im_PACKET(_perso, "1101");
						return;
					}
		...
		if(_perso.get_isTradingWith() < 0)
		{
			switch(packet.charAt(3))
			{
				...
				case '+'://Mettre un objet en vente
					...
					{
						GestorSalida.GAME_SEND_Im_PACKET(_perso, "058");
						return;
					}
					if(_perso.get_kamas() < taxe)
					{
						GestorSalida.GAME_SEND_Im_PACKET(_perso, "176");
						return;
					}
	...
	private void Exchange_accept()
	{
		...
		Jugador target = Mundo.getPersonnage(_perso.get_isTradingWith());
		if(target == null)return;
	...
	private void Exchange_onBuyItem(String packet)
	{
        ...
        if (_perso.get_isTradingWith() > 0) 
        {
            Jugador seller = Mundo.getPersonnage(_perso.get_isTradingWith());
            if (seller != null) 
            {
            	int itemID = 0;
            	int qua = 0;
	            ...
	            {
	            	if(Mundo.getSeller(seller.get_curCarte().get_id()) != null && Mundo.getSeller(seller.get_curCarte().get_id()).contains(seller.get_GUID()))
	        		{
	        			Mundo.removeSeller(seller.get_GUID(), seller.get_curCarte().get_id());
	        			GestorSalida.GAME_SEND_ERASE_ON_MAP_TO_MAP(seller.get_curCarte(), seller.get_GUID());
	        			Exchange_finish_buy();
	        		}
	            }
            }
	...
	private void Exchange_finish_buy()
	{
		...
		if(_perso.get_isTradingWith() > 0)
		{
			Jugador p = Mundo.getPersonnage(_perso.get_isTradingWith());
			if(p != null)
			{
				if(p.isOnline())
				{
		...
		if(_perso.get_isOnPercepteurID() != 0)
		{
			Recaudador perco = Mundo.getPerco(_perso.get_isOnPercepteurID());
			if(perco == null) return;
			for(Jugador z : Mundo.getGuild(perco.get_guildID()).getMembers())
			{
				if(z.isOnline())
				{
					GestorSalida.GAME_SEND_gITM_PACKET(z, Recaudador.parsetoGuild(z.get_guild().get_id()));
					...
					str += "|.|"+ Mundo.getCarte((short)perco.get_mapID()).getX()+"|"+ Mundo.getCarte((short)perco.get_mapID()).getY()+"|";
					str += _perso.get_name()+"|";
					str += perco.get_LogXp();
			...
			GestorSalida.GAME_SEND_ERASE_ON_MAP_TO_MAP(_perso.get_curCarte(), perco.getGuid());
			perco.DelPerco(perco.getGuid());
			GestorSQL.eliminar_recaudador(perco.getGuid());
	...
	private void Exchange_start(String packet)
	{
		if(packet.substring(2,4).equals("11"))//Ouverture HDV achat
		{
			...
			Mercadillo toOpen = Mundo.getHdv(_perso.get_curCarte().get_id());
		...
		switch(packet.charAt(2))
		{
			case '0'://Si NPC
				try
				{
					...
					NPCModelo.NPC npc = _perso.get_curCarte().getNPC(npcID);
					if(npc == null)return;
			...
			case '1'://Si joueur
				...
				Jugador target = Mundo.getPersonnage(guidTarget);
				if(target == null )
				{
            ...
            case '4'://StorePlayer
				...
				if(_perso.get_isTradingWith() > 0 || _perso.get_fight() != null || _perso.is_away())return;
				Jugador seller = Mundo.getPersonnage(pID);
	...
	private void Environement_emote(String packet)
	{
		...
		GestorSalida.GAME_SEND_eUK_PACKET_TO_MAP(_perso.get_curCarte(), _perso.get_GUID(), _perso.emoteActive());
	}
	...
	private void Environement_change_direction(String packet)
	{
		try
		{
			...
			GestorSalida.GAME_SEND_eD_PACKET_TO_MAP(_perso.get_curCarte(),_perso.get_GUID(),dir);
		}catch(NumberFormatException e){return;};
	}
		...
		try
		{
			switch(packet.charAt(1))
			{
				case 'D'://Dï¿½tails d'un combat (liste des combats)
					...
					if(key == -1)return;
					GestorSalida.GAME_SEND_FIGHT_DETAILS(_out,_perso.get_curCarte().get_fights().get(key));
				break;
				...
				case 'L'://Lister les combats
					GestorSalida.GAME_SEND_FIGHT_LIST_PACKET(_out, _perso.get_curCarte());
				break;
		...
		switch(packet.charAt(2))
		{
			case 'A': //Absent
				if(_perso._isAbsent)
				{
					...
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "038");
				...
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "037");
			...
			case 'I': //Invisible
				...
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "051");
					_perso._isInvisible = false;
				}
				else
				{
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "050");
	...
	private void Basic_chatMessage(String packet)
	{
		...
		if(_perso.isMuted())
		{
			GestorSalida.GAME_SEND_Im_PACKET(_perso, "1124;"+_perso.get_compte()._muteTimer.getInitialDelay());//FIXME
			return;
		}
		...
		switch(packet.charAt(2))
		{
			case '*'://Canal noir
				...
				if(msg.charAt(0) == '.')
				{
					...
					if (msg.length() > 6 && msg.substring(1, 7).equalsIgnoreCase("astrub"))
					{
						...
						{
							_perso.teleport((short)7411, 369);
					...
					_perso.teleport(MainServidor.CONFIG_MAP_SHOP, MainServidor.CONFIG_CELL_SHOP);
					return;
				}else
					    ...
					    if(msg.length() > 6 && msg.substring(1, 7).equalsIgnoreCase("enclos"))
					    {
					    	if (_perso.get_fight() != null || !MainServidor.CONFIG_ALLOW_PLAYER_COMMANDS)
					    	{
					    		...
					    		_perso.teleport(MainServidor.CONFIG_ENCLOS_MAP, MainServidor.CONFIG_CELL_ENCLOS);
					    	return;
					    }else
					...
					if(msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("pvm"))
					{
						...
						_perso.teleport(MainServidor.CONFIG_MAP_PVM, MainServidor.CONFIG_CELL_PVM);
						return;
					}
					if(msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("pvp"))
					{
						...
						_perso.teleport(MainServidor.CONFIG_MAP_PVP, MainServidor.CONFIG_CELL_PVP);
						return;
					}
						...
						if(msg.length() > 6 && msg.substring(1, 7).equalsIgnoreCase("parcho"))
					      {
					        ...
					        if(nbreElement == 0)
					        {
					            GestorSalida.GAME_SEND_Im_PACKET(_perso, "116;<i>System</i>~Vous avez déjà plus de 100 partout !");
					        }
					        else
					        {
					            GestorSalida.GAME_SEND_STATS_PACKET(_perso);
					            GestorSalida.GAME_SEND_Im_PACKET(_perso, "116;<i>System</i>~Vous êtes parcho 101 en " + element + " !");
					        }
					        return;
				...
				if(_perso.get_fight() == null)
					GestorSalida.GAME_SEND_cMK_PACKET_TO_MAP(_perso.get_curCarte(), "", _perso.get_GUID(), _perso.get_name(), msg);
				else
			...
			case '$'://Canal groupe
				...
				msg = packet.split("\\|",2)[1];
				GestorSalida.GAME_SEND_cMK_PACKET_TO_GROUP(_perso.getGroup(), "$", _perso.get_GUID(), _perso.get_name(), msg);
			break;
			...
			case ':'://Canal commerce
				...
				if((l = System.currentTimeMillis() - _timeLastTradeMsg) < MainServidor.FLOOD_TIME)
				{
					...
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "0115;"+((int)Math.ceil(l)+1));
					return;
				}
			...
			case '?'://Canal recrutement
				...
				if((j = System.currentTimeMillis() - _timeLastRecrutmentMsg) < MainServidor.FLOOD_TIME)
				{
					...
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "0115;"+((int)Math.ceil(j)+1));
					return;
				}
			...
			case '!'://Alignement
				...
				if((k = System.currentTimeMillis() - _timeLastAlignMsg) < MainServidor.FLOOD_TIME)
				{
					...
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "0115;"+((int)Math.ceil(k)+1));
					return;
				}
			...
			case '^':// Canal Incarnam 
				...
				if((x = System.currentTimeMillis() - _timeLastIncarnamMsg) < MainServidor.FLOOD_TIME)
				{
					...
					GestorSalida.GAME_SEND_Im_PACKET(_perso, "0115;"+((int)Math.ceil(x)+1));
					return; 
				} 
			...
			default:
				...
				{
					Jugador target = Mundo.getPersoByName(nom);
					...
					{
						GestorSalida.GAME_SEND_Im_PACKET(_perso, "114;"+target.get_name());
						return;
					}
	...
	{
			packet = packet.substring(2);
			Jugador T = Mundo.getPersoByName(packet);
			if(T == null) return;
	...
	private void Game_onLeftFight(String packet)
	{
		...
		{
			Jugador target = Mundo.getPersonnage(targetID);
			...
			case 1://Deplacement
				if(isOk)
				{
					...
					if(_perso.get_fight() == null)
					{
						...
						Case nextCell = _perso.get_curCarte().getCase(GestorEncriptador.cellCode_To_ID(path.substring(path.length()-2)));
						Case targetCell = _perso.get_curCarte().getCase(GestorEncriptador.cellCode_To_ID(GA._packet.substring(GA._packet.length()-2)));
						...
						if(targetCell.getObject() != null)
						{
							...
							{
								Constantes.applyPlotIOAction(_perso,_perso.get_curCarte().get_id(),targetCell.getID());
							}
							//Statues phoenix
	...
	private void Game_on_GI_packet() 
	{
		...
		{
			//Only percepteur
			GestorSalida.GAME_SEND_MAP_GMS_PACKETS(_perso.get_curCarte(), _perso);
			GestorSalida.GAME_SEND_GDK_PACKET(_out);
			return;
		}
		//Enclos
		GestorSalida.GAME_SEND_Rp_PACKET(_perso, _perso.get_curCarte().getMountPark());
		//Maisons
		House.LoadHouse(_perso, _perso.get_curCarte().get_id());
		//Objets sur la carte
		GestorSalida.GAME_SEND_MAP_GMS_PACKETS(_perso.get_curCarte(), _perso);
		GestorSalida.GAME_SEND_MAP_MOBS_GMS_PACKETS(_perso.get_compte().getGameThread().get_out(), _perso.get_curCarte());
		GestorSalida.GAME_SEND_MAP_NPCS_GMS_PACKETS(_perso,_perso.get_curCarte());
		GestorSalida.GAME_SEND_MAP_PERCO_GMS_PACKETS(_out,_perso.get_curCarte());
		GestorSalida.GAME_SEND_MAP_OBJECTS_GDS_PACKETS(_out,_perso.get_curCarte());
		GestorSalida.GAME_SEND_GDK_PACKET(_out);
		GestorSalida.GAME_SEND_MAP_FIGHT_COUNT(_out, _perso.get_curCarte());
		GestorSalida.GAME_SEND_MERCHANT_LIST(_perso, _perso.get_curCarte().get_id());
		//Les drapeau de combats
		Pelea.FightStateAddFlag(_perso.get_curCarte(), _perso);
		//items au sol
	...
	private void parseGameActionPacket(String packet)
	{
		...
		switch(actionID)
		{
			...
			case 618://Mariage oui
				...
				GestorSalida.GAME_SEND_cMK_PACKET_TO_MAP(_perso.get_curCarte(), "", _perso.get_GUID(), _perso.get_name(), "Oui");
				...
				{
					Mundo.PriestRequest((Mundo.getMarried(0)==_perso? Mundo.getMarried(1): Mundo.getMarried(0)), (Mundo.getMarried(0)==_perso? Mundo.getMarried(1).get_curCarte(): Mundo.getMarried(0).get_curCarte()), _perso.get_isTalkingWith());
				}
			break;
			case 619://Mariage non
				_perso.setisOK(0);
				GestorSalida.GAME_SEND_cMK_PACKET_TO_MAP(_perso.get_curCarte(), "", _perso.get_GUID(), _perso.get_name(), "Non");
	...
	private void game_perco(String packet)
	{
		try
		{
			...
			if(target.get_Exchange())
			{
				...
				GestorSalida.GAME_SEND_Im_PACKET(_perso, "1180");
				return;
			}
			GestorSalida.GAME_SEND_GA_PACKET_TO_MAP(_perso.get_curCarte(),"", 909, _perso.get_GUID()+"", id+"");
	...
	private void game_aggro(String packet)
	{
		try
		{
			...
			if(_perso.get_fight() != null)return;
			int id = Integer.parseInt(packet.substring(5));
			Jugador target = Mundo.getPersonnage(id);
			...
			if(target.get_align() == 0) 
			{
				...
				GestorSalida.GAME_SEND_Im_PACKET(_perso, "084;1");
			}

			_perso.toggleWings('+');
			GestorSalida.GAME_SEND_GA_PACKET_TO_MAP(_perso.get_curCarte(),"", 906, _perso.get_GUID()+"", id+"");
			_perso.get_curCarte().newFight(_perso, target, Constantes.FIGHT_TYPE_AGRESSION);
		}catch(Exception e){};
	}
	...
	private void game_join_fight(String packet)
	{
		...
		if(infos.length == 1)
		{
			try
			{
				Pelea F = _perso.get_curCarte().getFight(Integer.parseInt(infos[0]));
	...
	private void game_accept_duel(String packet)
	{
		...
		GestorSalida.GAME_SEND_MAP_START_DUEL_TO_MAP(_perso.get_curCarte(),_perso.get_duelID(),_perso.get_GUID());
		Pelea fight = _perso.get_curCarte().newFight(Mundo.getPersonnage(_perso.get_duelID()),_perso, Constantes.FIGHT_TYPE_CHALLENGE);
	...
	private void game_cancel_duel(String packet)
	{
		try
		{
			...
			GestorSalida.GAME_SEND_CANCEL_DUEL_TO_MAP(_perso.get_curCarte(),_perso.get_duelID(),_perso.get_GUID());
	...
	private void game_ask_duel(String packet)
	{
		...
		try
		{
			...
			Jugador Target = Mundo.getPersonnage(guid);
			if(Target == null) return;
			...
			GestorSalida.GAME_SEND_MAP_NEW_DUEL_TO_MAP(_perso.get_curCarte(),_perso.get_GUID(),guid);
		}catch(NumberFormatException e){return;}
	}
	...
	private void game_parseDeplacementPacket(GameAction GA)
	{
		...
		if(_perso.get_fight() == null)
		{
			...
			{
				GestorSalida.GAME_SEND_Im_PACKET(_perso, "112");
			...
			if(result == -1000)
			{
				...
				path = GestorEncriptador.getHashedValueByInt(_perso.get_orientation())+ GestorEncriptador.cellID_To_Code(_perso.get_curCell().getID());
			}
			//On sauvegarde le path dans la variable
			GA._args = path;
			
			GestorSalida.GAME_SEND_GA_PACKET_TO_MAP(_perso.get_curCarte(), ""+GA._id, 1, _perso.get_GUID()+"", "a"+ GestorEncriptador.cellID_To_Code(_perso.get_curCell().getID())+path);
			addAction(GA);
	...
	private void Object_obvijevan_changeApparence(String packet)
	{
		...
		try
		{
			...
			if (pos != -1) GestorSalida.GAME_SEND_ON_EQUIP_ITEM(_perso.get_curCarte(), _perso);
	}
