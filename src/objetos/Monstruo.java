...
public class Monstruo
{
	...
	public static class MobGroup
	{
		...
		public MobGroup(int Aid, int Aalign, ArrayList<MobGrade> possibles, Mapa Map, int cell, int maxSize)
		{
			...
			aggroDistance = Constantes.getAggroByLevel(maxLevel);
			
			if(align != Constantes.ALIGNEMENT_NEUTRE)
				aggroDistance = 15;
		...
		public MobGroup(int Aid, int cID, String groupData)
		{
			int maxLevel = 0;
			id = Aid;
			align = Constantes.ALIGNEMENT_NEUTRE;
			cellID = cID;
			aggroDistance = Constantes.getAggroByLevel(maxLevel);
			isFix = true;
			_creationDate = System.currentTimeMillis();
	...
	public static class MobGrade
	{
		...
		public MobGrade(Monstruo aTemp, int Agrade, int Alevel, int aPA, int aPM, String Aresist, String Astats, String Aspells, int pdvMax, int aInit, int xp)
		{
			...
			stats.put(Constantes.STATS_ADD_INTE, intell);
			stats.put(Constantes.STATS_ADD_CHAN, chance);
			stats.put(Constantes.STATS_ADD_AGIL, agilite);
			stats.put(Constantes.STATS_ADD_RP_NEU, RN);
			stats.put(Constantes.STATS_ADD_RP_FEU, RF);
			stats.put(Constantes.STATS_ADD_RP_EAU, RE);
			stats.put(Constantes.STATS_ADD_RP_AIR, RA);
			stats.put(Constantes.STATS_ADD_RP_TER, RT);
			stats.put(Constantes.STATS_ADD_AFLEE, AF);
			stats.put(Constantes.STATS_ADD_MFLEE, MF);
		...
		private MobGrade(Monstruo template2, int grade2, int level2, int pdv2, int pdvmax2, int aPA, int aPM, Map<Integer, Integer> stats2, Map<Integer, SortStats> spells2, int xp)
		{
			template = template2;
			grade = grade2;
			level = level2;
		...
		public void modifStatByInvocator(Fighter caster)
		{
			...
			PDV = (PDVMAX)*coef;
			PDVMAX = PDV;
			int force = stats.get(Constantes.STATS_ADD_FORC)*coef;
			int intel = stats.get(Constantes.STATS_ADD_INTE)*coef;
			int agili = stats.get(Constantes.STATS_ADD_AGIL)*coef;
			int sages = stats.get(Constantes.STATS_ADD_SAGE)*coef;
			int chanc = stats.get(Constantes.STATS_ADD_CHAN)*coef;
			stats.put(Constantes.STATS_ADD_FORC, force);
			stats.put(Constantes.STATS_ADD_INTE, intel);
			stats.put(Constantes.STATS_ADD_AGIL, agili);
			stats.put(Constantes.STATS_ADD_SAGE, sages);
			stats.put(Constantes.STATS_ADD_CHAN, chanc);
		}
	}

	public Monstruo(int Aid, int agfx, int Aalign, String Acolors, String Agrades, String Aspells, String Astats, String aPdvs, String aPoints, String aInit, int mK, int MK, String xpstr, int IAtype, boolean capturable)
	{
		ID = Aid;
		gfxID = agfx;
		align = Aalign;
