...
public class Accion {
	...
	public void apply(Personaje perso, Personaje target, int itemID, int cellid) {
		...
		switch(_id) {
			...
			case 18://Téléportation chez sois
				if(Casas.AlreadyHaveHouse(perso))//Si il a une maison
				{
					...
					if (perso.hasItemTemplate(obj.getTemplate().getID(), 1))
					{
						...
						if(h == null) return;
						perso.teletransportar((short)h.get_mapainterior(), h.get_celdainterior());
					}
				}
			break;
			...
			case 106://Desbloquear en celda libre mismo mapa
				if (perso.getPelea() != null)
					return;
				perso.teletransportar(perso.getActualMapa().getID(), perso.getActualMapa().getRandomFreeCellID());
				break;
	...
	public static class AccionIntercambiar<T> {
		...
		public AccionIntercambiar(byte type, T value) {
			this.type = type;
			this.value = value;
		}
