	...
	private synchronized void Object_move(String packet)
	{
		...
		try
		{
			...
			if ((objGUID == 9234) || (objGUID == 9233) || (objGUID == 9255) || (objGUID == 9256))
			{
				...
				_perso.removeItem(obj.getID(), 1, false, false); // on enlï¿½ve l'existance de l'obvi en lui-mï¿½me
				GestorSalida.enviar(_perso, "OR" + obj.getID()); // on le prï¿½cise au client
